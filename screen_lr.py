...
def main(description, seed, export, export_file_path):
	...
	for i in learning_rates:
		...
		if "backprop" in description: 
			...
			acc = net.train_model('backprop', i, criterion, train_loader, debug=False, num_epochs=num_epochs, verbose=True, device=DEVICE)
			...
			if description == "dend_temp_contrast_learned_bias":
				...
				net = Net(nn.ReLU, X_train.shape[1], [128, 32], num_classes, description=description, use_bias=True, learn_bias=False).to(DEVICE)
			acc = net.train_model('dend_temp_contrast', i, criterion, train_loader, debug=False, num_epochs=num_epochs, verbose=True, device=DEVICE)
